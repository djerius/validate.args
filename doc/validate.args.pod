...
then the specifications should be bundled as a list:
  ...
  { specs_for_arg_1 , specs_for_arg_2 }
...
must meet.  The specification takes the form of a table, where each
key-value pair represents a type of constraint and its
parameters.  Most typically these tables are what is passed in the
list of specifications described above; see however, L</Mutating
Validation Specifications>.

Multiple constraints may be specified for each argument. They are
applied in random order.  The caller may provide constraints
...
The validation specifications are usually passed as tables.  It is
sometimes necessary to be able to construct validations on the fly
(much as does the C<vtable> constraint).  In such cases the validation
specification should be specified as a I<function> which takes a
single option - the argument - and returns two values.  The first
value is a boolean indicating success or failure.  In the case of
success the second value is a table containing the actual validation
specification.  In the case of failure it is a message to be returned
to the caller.
