...
are identical to those described above. Typically the B<validate_tbl> method or
function is used.
...
This specifies the expected type of the element. It may be either a single
...
If an error occurs during validation, the C<postcall> function
will not be called.

=item *

Specification validation is done before the C<precall> function is
...
This indicates that the element is an array whose members must
...
If C<true>, each member of the element array is validated.

=item a table of options

In addition to validating the members of the input array, further
...
elements.  These are specified as special "arguments" in the
