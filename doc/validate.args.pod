...
be a table.  This is how nested tables of arguments are validated. For example,
the following validation specificaton:
...
be set globally for all validations using B<opts()>.  Values set using
B<opts> are used as defaults.
...
If true, the Lua B<error()> function will be called the case of
...
If this is true and C<allow_extra> is also true, then any extra
